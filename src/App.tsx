          ...
          {isDetailView ? (
            ...
            <div className="w-full">
              ...
              {selectedProduct.detailImages ? (
                selectedProduct.detailImages.split(',').map((url: string, idx: number) => (
            ...
            <div className="flex flex-col h-full justify-between">
              <div>
                ...
                {selectedProduct.detailImages && (
                  <button type="button" onClick={() => setIsDetailView(true)} className="w-full py-2.5 mb-4 bg-brand/10 text-brand-dark text-xs font-bold rounded-xl hover:bg-brand/20 transition-all border border-brand/20">
                    상세 상품 이미지 더 보기
                  </button>
                )}
                ...
                <div className="space-y-1.5 mb-6">
                  ...
                  <div className="flex items-center justify-between p-2 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div className="flex items-center gap-1">
                      <button type="button" onClick={() => setQuantity(prev => Math.max(1, prev - 1))} className="w-8 h-8 bg-white rounded-lg flex items-center justify-center border border-gray-100 text-gray-500 hover:bg-gray-100"><Minus size={14} /></button>
                      <button type="button" onClick={() => setQuantity(prev => prev + 1)} className="w-8 h-8 bg-white rounded-lg flex items-center justify-center border border-gray-100 text-gray-500 hover:bg-gray-100"><Plus size={14} /></button>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
                ...
                <button type="button" onClick={() => { if (productOptions.length > 0 && !selectedOption) { alert("옵션을 선택해 주세요."); return; } setIsOrderView(true); }} className="bg-ink text-white text-xs font-extrabold px-6 py-3.5 rounded-xl hover:bg-brand hover:text-black shadow-md flex items-center gap-1.5 transition-all">
            ...
            <div className="w-full">
              ...
              <form onSubmit={handleOrderSubmit} className="space-y-3.5">
                ...
                <button type="submit" disabled={isSubmitting} className={`w-full py-4 text-xs font-extrabold rounded-xl transition-all mt-2 shadow-sm ${isSubmitting ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-ink text-white hover:bg-brand hover:text-ink'}`}>
